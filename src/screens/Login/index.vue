<template>
  <div class="login-screen">
    <form class="login-screen__form" @submit="submitForm">
      <div class="login-screen__form-header">
        <ButtonComponent @click.prevent="goBackButtonHandler" class="login-screen__form-back-button" icon="arrow_back" />
      ...
      <h3 v-if="isLogin" class="login-screen__title">
        Вход
      </h3>
      <h3 v-if="isRegister" class="login-screen__title">
        Вход
      </h3>
      <h3 v-if="isForget" class="login-screen__title">
        Забыли пароль?
      </h3>

      <TextFieldComponent v-model="loginValue" class="login-screen__input" placeholder="Номер телефона"
        type="phoneNumber" />

      <TextFieldComponent v-if="isLogin || isRegister" v-model="passwordValue"
        class="login-screen__input login-screen__input--password" placeholder="Пароль" type="password" />

      <span class="login-screen__forgot-password" @click="switchScreen('forget')" v-if="isLogin">
        Забыли пароль?
      </span>

      <div class="login-screen__footer">
        <ButtonComponent @click="switchScreen('register')"
          class="login-screen__footer-button login-screen__footer-button--registration" text="Зарегистрироваться"
          v-if="isLogin" type="base" />
